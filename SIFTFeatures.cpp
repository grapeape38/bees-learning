...
void SIFTFeatures::computeFeatures(Runtime::Buffer<uint8_t,3> input) {   
    keypoints.clear();
    features.clear();
    const int s = 5;
    //int num_scales = 2;
    //for(int i = 0; i < num_scales; i++) {
        ...
        ft_index.assign(width, std::vector<int>(height, 0));
...
inline Feature computeDescript(int k, int x, int y, int maxX, int maxY, int th, float sc,
    ...
    for (const auto &v : keypoints) {
        ...
        for (int i = 0; i < 36; i++) {
            if (maxTh - orient_hist[i] < 0.2f * maxTh) {
                ...
                if (th > 180) th -= 360;
                kp++;
                features.push_back(computeDescript(k,x,y,maxX,maxY,th,sc,mag,theta));
                if (maxTh == orient_hist[i])
                    ft_index[x][y] = features.size() - 1;
            }
        }
        if (kp > 1) {
            num_orient++;
